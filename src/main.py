...
def delete_menu(storage: JSONStorage) -> None:
    ...
    while True:
        ...
        if choice == "1":
            ...
            confirm = (
                input(f"Удалить все {len(planes_to_delete)} самолетов? (да/нет): ").strip().lower()
            )
            ...
            try:
                altitude_str = input("Введите минимальную высоту (м): ").strip()
                if not altitude_str:
                    print("Ошибка: высота не может быть пустой")
                    continue
                ...
                confirm = (
                    input(f"Удалить все {len(to_delete)} самолетов? (да/нет): ").strip().lower()
                )
                if confirm in ("да", "yes", "y"):
            ...
            callsign = (
                input("Введите позывной/наименование самолета для удаления: ").strip().upper()
            )
            if not callsign:
            ...
            print(to_delete[0])
            confirm = input("Удалить этот самолет? (да/нет): ").strip().lower()
            if confirm in ("да", "yes", "y"):
...
def user_interaction() -> None:
    ...
    while True:
        ...
        try:
            ...
            print(
                f"Страна '{country}' найдена! Координаты: "
                f"широта {coords[0]:.2f}° - {coords[1]:.2f}°, "
                f"долгота {coords[2]:.2f}° - {coords[3]:.2f}°"
            )
            break

        except ValueError:
            print(
                f"Ошибка: страна '{country}' не найдена. Проверьте правильность написания и попробуйте снова."
            )
            continue
    ...
    try:
        ...
        while True:
            try:
                top_n_str = input(
                    f"\nВведите количество самолетов для вывода в топ N по высоте (до {MAX_TOP}): "
                ).strip()
        ...
        filter_country = (
            input("\nХотите отфильтровать самолеты по стране регистрации? (да/нет): ")
            .strip()
            .lower()
        )
        if filter_country in ("да", "yes", "y"):
            countries_str = input(
                "Введите страны через запятую (например, United States, France): "
            ).strip()
